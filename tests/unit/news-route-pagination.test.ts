...
describe("GET /api/news pagination (#140)", () => {
  ...
  it("rejects an offset past the ceiling instead of serving a clamped page", async () => {
    const response = await getNews(request("http://localhost/api/news?limit=20&offset=999999999"));
    const data = await body(response);

    expect(response.status).toBe(400);
    expect(data.message).toBe("offset exceeds maximum of 10000");
    // A clamped page would come back with hasMore true and a paginating client
    // would walk page 10000 forever, so the database is never asked at all.
    expect(getPaginatedFiltered).not.toHaveBeenCalled();
  });

  it("serves the deepest allowed offset and rejects the one past it", async () => {
    getPaginatedFiltered.mockResolvedValue({ articles: [], total: 20000, hasMore: true });

    const allowed = await getNews(request("http://localhost/api/news?limit=20&offset=10000"));
    ...
    const refused = await getNews(request("http://localhost/api/news?limit=20&offset=10001"));

    expect(refused.status).toBe(400);
    expect((await body(refused)).message).toBe("offset exceeds maximum of 10000");
    expect(getPaginatedFiltered).not.toHaveBeenCalled();
  });
