...
function compareTranslations() {
  ...
  for (const lang of LANGUAGES) {
    ...
    report.results[lang] = {
      ...
      completeness: `${(
        ((referenceKeys.length - missingKeys.length - emptyKeys.length) / referenceKeys.length) *
          100
      ).toFixed(2)}%`,
    };
  ...
  for (const lang of LANGUAGES) {
    ...
    if (result.error) {
      ...
      console.log(
        `${lang}: ${result.completeness} complete (${result.missingCount} missing, ${result.emptyCount} empty)`
      );
    }
  }
