...
export class ToolsRepository extends BaseRepository<ToolData> {
  ...
  private hasWarnedAboutMissingFile = false;
  private hasLoggedDataSource = false;

  constructor() {
    super();
    // Log data source once on initialization
    if (!this.hasLoggedDataSource) {
      console.log(`[ToolsRepository] Using ${this.useDatabase ? 'DATABASE' : 'JSON'} as data source`);
      this.hasLoggedDataSource = true;
    }
  }
  ...
  async updateScoring(
    ...
    // For JSON, update in memory
    return this.updateInJson(id, scoringData);
  }
  ...
  private loadJsonData(): { tools: ToolData[] } {
    ...
    try {
      ...
      this.hasWarnedAboutMissingFile = false;
      return this.jsonCache!;
    } catch (error) {
      // Only warn once about missing file to avoid console spam
      if (!this.hasWarnedAboutMissingFile) {
        if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
          console.warn(`Tools JSON file not found at ${this.jsonPath}, using empty array`);
        } else {
          console.error("Error loading tools JSON:", error);
        }
        this.hasWarnedAboutMissingFile = true;
      }
      return { tools: [] };
    }
  }
