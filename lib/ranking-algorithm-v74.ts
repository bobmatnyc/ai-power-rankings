/**
 * Algorithm v7.4: Data-Driven Confidence Scoring with Missing Data Penalty
 *
 * Core principle: "No data = Lower confidence = Lower score"
 *
 * NEW in v7.4:
 * - Data completeness scoring system
 * - Confidence multiplier based on real metrics availability
 * - Penalizes tools with limited verification data
 * - Rewards tools with comprehensive, verifiable metrics
 *
 * Key improvements over v7.3:
 * - Tools with real-world metrics (GitHub, VS Code, npm, revenue) rank higher
 * - Tools with limited data (only descriptions) are appropriately penalized
 * - Confidence multiplier ranges from 0.7 (no data) to 1.0 (complete data)
 * - Maintains all v7.3 enhancements (tiebreakers, differentiation, etc.)
 *
 * Success criteria:
 * - Data-backed tools (Cursor, Copilot, Claude Code) rank higher
 * - Unverified tools (Jules, Refact.ai) drop appropriately
...
export interface RankingWeightsV74 {
...
export const ALGORITHM_V74_WEIGHTS: RankingWeightsV74 = {
  agenticCapability: 0.08,      // ↓ from 0.35 - reduce theory-based scoring
  innovation: 0.08,              // ↓ from 0.10 - reduce hype-based scoring
  technicalPerformance: 0.10,    // ↓ from 0.10 - maintain benchmark value
  developerAdoption: 0.22,       // ↑ from 0.125 - MAJOR INCREASE for real users
  marketTraction: 0.18,          // ↑ from 0.125 - MAJOR INCREASE for market validation
...
export interface ToolMetricsV74 {
  tool_id: string;
  name: string;
  slug: string;
  category?: string;
 ...
 */
function calculateDescriptionQuality(metrics: ToolMetricsV74): number {
 ...
 */
function calculatePricingTier(metrics: ToolMetricsV74): number {
 ...
 */
function calculateCompanyBacking(metrics: ToolMetricsV74): number {
 ...
 */
function calculateMaturityBonus(metrics: ToolMetricsV74): number {
...
export class RankingEngineV74 {
  constructor(private weights: RankingWeightsV74 = ALGORITHM_V74_WEIGHTS) {}
   ...
   */
  private getData(metrics: ToolMetricsV74): {
    info: any;
    metrics: any;
  } {
   ...
   */
  private calculateAgenticCapability(metrics: ToolMetricsV74): number {
   ...
   */
  private calculateInnovation(metrics: ToolMetricsV74): number {
   ...
   */
  private calculateTechnicalPerformance(metrics: ToolMetricsV74): number {
   ...
   */
  private calculateDeveloperAdoption(metrics: ToolMetricsV74): number {
   ...
   */
  private calculateMarketTraction(metrics: ToolMetricsV74): number {
   ...
   */
  private calculateDevelopmentVelocity(metrics: ToolMetricsV74): number {
    let score = 50; // Base
   ...
   */
  private calculatePlatformResilience(metrics: ToolMetricsV74): number {
    let score = 50; // Base score
   ...
   */
  private calculateTiebreakers(metrics: ToolMetricsV74): {
    featureCount: number;
    descriptionQuality: number;
    pricingTier: number;
    alphabeticalOrder: number;
