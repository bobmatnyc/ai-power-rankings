...
import { getNewsRepo, getToolsRepo, initializeRepositories } from "../../src/lib/json-db";
...
class NewsMigrator {
            ...
            if (newsItem.related_tools && Array.isArray(newsItem.related_tools)) {
              for (const toolSlug of newsItem.related_tools) {
                ...
                if (toolId) {
                  ...
                  const toolByName = existingTools.find(
                    (t) =>
  ...
  private async migrateFromIncomingSamples(): Promise<void> {
    ...
    try {
      ...
      const sampleFiles = ["sample-news-upload.json", "sample-news-batch-2.json"];
        ...
        if (Array.isArray(sampleData)) {
          for (const newsItem of sampleData) {
            try {
              ...
              const article: NewsArticle = {
                id:
                  newsItem.id || `sample-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
                slug,
  ...
  private reportResults(): void {
    ...
    const resultsPath = path.join(process.cwd(), "data", "json", "news-migration-results.json");
