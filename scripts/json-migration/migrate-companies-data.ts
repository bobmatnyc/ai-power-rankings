...
import { getCompaniesRepo, initializeRepositories } from "../../src/lib/json-db";
...
class CompaniesMigrator {
        ...
        for (const companyItem of companiesData) {
          try {
            ...
            const company: Company = {
              id:
                companyItem.id ||
                `company-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
              slug,
  ...
  private async migrateFromImportFiles(): Promise<void> {
    ...
    try {
      ...
      const importFiles = ["companies-import.json", "company-tool-updates-june-2025.json"];
      ...
      for (const importFile of importFiles) {
        ...
        for (const companyItem of companiesToProcess) {
          try {
            ...
            const company: Company = {
              id:
                companyItem.id || `import-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
              slug,
              name: companyItem.name || "Unknown Company",
              description: companyItem.description,
              ...
              founded:
                companyItem.founded ||
                (companyItem.founded_year ? companyItem.founded_year.toString() : undefined),
              headquarters: companyItem.headquarters,
