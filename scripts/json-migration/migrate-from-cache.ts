...
import type { Company, NewsArticle, RankingPeriod, Tool } from "../../src/lib/json-db/schemas";
...
class CacheMigrator {
      ...
      for (const cachedTool of cachedTools) {
        try {
          const jsonTool: Tool = {
            ...
            status:
              cachedTool.status === "active"
                ? "active"
                : cachedTool.status === "inactive"
                  ? "inactive"
                  : cachedTool.status === "deprecated"
                    ? "deprecated"
                    : "active",
    ...
    try {
      ...
      if (cacheData.rankings && Array.isArray(cacheData.rankings)) {
        const rankings = cacheData.rankings.map((ranking: any, index: number) => ({
          ...
          factor_scores: {
            agentic_capability:
              ranking.agentic_capability || ranking.factor_scores?.agentic_capability || 0,
            innovation: ranking.innovation || ranking.factor_scores?.innovation || 0,
            technical_performance:
              ranking.technical_performance || ranking.factor_scores?.technical_performance || 0,
            developer_adoption:
              ranking.developer_adoption || ranking.factor_scores?.developer_adoption || 0,
            market_traction: ranking.market_traction || ranking.factor_scores?.market_traction || 0,
            business_sentiment:
              ranking.business_sentiment || ranking.factor_scores?.business_sentiment || 0,
            development_velocity:
              ranking.development_velocity || ranking.factor_scores?.development_velocity || 0,
            platform_resilience:
              ranking.platform_resilience || ranking.factor_scores?.platform_resilience || 0,
          },
        ...
        for (const cachedArticle of cacheData.articles) {
          try {
            const jsonArticle: NewsArticle = {
              ...
              published_date:
                cachedArticle.published_date || cachedArticle.createdAt || new Date().toISOString(),
              source: cachedArticle.source || undefined,
              source_url: cachedArticle.source_url || undefined,
              tags: cachedArticle.tags || undefined,
              tool_mentions:
                cachedArticle.tool_mentions || cachedArticle.related_tools || undefined,
  ...
  private reportResults(): void {
    ...
    const resultsPath = path.join(process.cwd(), "data", "json", "migration-results.json");
