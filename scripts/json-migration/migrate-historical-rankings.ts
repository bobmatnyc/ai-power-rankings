...
import type { RankingPeriod, RankingEntry } from '../../src/lib/json-db/schemas';
...
class HistoricalRankingsMigrator {
    ...
    try {
      ...
      for (const cacheEntry of rankingCache) {
        try {
          ...
          const periodMeta = rankingPeriods.find((p: any) => 
            this.formatPeriod(p.period) === period || 
            this.formatPeriod(p.created_at) === period
          );
    ...
    try {
      ...
      if (historicalData.rankings && Array.isArray(historicalData.rankings)) {
        ...
        if (existingPeriod && historicalData.rankings.length > existingPeriod.rankings.length) {
          logger.info(`Upgrading ${period} with more comprehensive data (${historicalData.rankings.length} vs ${existingPeriod.rankings.length} tools)`);
          
          const rankings: RankingEntry[] = historicalData.rankings.map((ranking: any, index: number) => {
            const toolId = this.findToolId(ranking, toolsMap, index);
  ...
  private calculateMovement(ranking: any): { previous_position?: number; change: number; direction: 'up' | 'down' | 'same' | 'new' } | undefined {
