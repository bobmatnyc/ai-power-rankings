...
class MigrationValidator {
  ...
  private async validateRankingPeriod(
    payload: any,
    period: any,
    ...
    try {
      ...
      for (const payloadRanking of payloadRankings) {
        const toolId =
          typeof payloadRanking.tool === "object" ? payloadRanking.tool.id : payloadRanking.tool;
        const jsonRanking = jsonRankingsMap.get(toolId);

        if (!jsonRanking) {
          result.validation_errors.push(
            `Period ${period.period}: missing ranking for tool ${toolId}`
          );
          result.success = false;
          continue;
        }
  ...
  private async saveReport(): Promise<void> {
    const reportPath = path.join(process.cwd(), "data", "json", "validation-report.json");
