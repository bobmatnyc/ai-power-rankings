...
/**
 * Add New AI Tools - October 2025
 *
 * Adds three new AI development tools to the database:
 ...
 *
 * Each tool includes comprehensive metadata and baseline scoring.
 */
...
async function addNewTools() {
  const db = getDb();
  console.log("🚀 Adding New AI Tools - October 2025\n");
  ...
  for (const toolDef of newTools) {
    try {
      console.log(`\n📦 Processing: ${toolDef.name}`);

      // Check if tool already exists
      const existing = await db
        .select()
        .from(tools)
        .where(eq(tools.slug, toolDef.slug))
        .limit(1);

      if (existing.length > 0) {
        console.log(`⚠️  Tool already exists: ${toolDef.name}`);
        skippedCount++;
        continue;
      }

      // Generate baseline scores
      const baselineScore = generateFactorScores(toolDef.overallScore);
      ...
      console.log(`✅ Added: ${toolDef.name}`);
      console.log(`   Slug: ${toolDef.slug}`);
      console.log(`   Category: ${toolDef.category}`);
      console.log(`   Overall Score: ${toolDef.overallScore}/100`);
      console.log(`   Baseline Scores:`);
      console.log(`     - Market Traction: ${baselineScore.marketTraction}`);
      console.log(`     - Technical Capability: ${baselineScore.technicalCapability}`);
      console.log(`     - Developer Adoption: ${baselineScore.developerAdoption}`);
      console.log(`     - Development Velocity: ${baselineScore.developmentVelocity}`);
      console.log(`     - Platform Resilience: ${baselineScore.platformResilience}`);
      console.log(`     - Community Sentiment: ${baselineScore.communitySentiment}`);

      addedCount++;
    } catch (error) {
      console.error(`❌ Error adding ${toolDef.name}:`, error);
      errorCount++;
    }
  }
  ...
  console.log(`   ⚠️  Skipped (already exist): ${skippedCount} tools`);
  console.log(`   ❌ Errors: ${errorCount} tools`);
  console.log(`   📦 Total processed: ${newTools.length} tools`);
  ...
  if (addedCount > 0) {
    ...
    for (const toolDef of newTools) {
      ...
      if (result.length > 0) {
        ...
        console.log(`  Launch: ${data.launchDate || "N/A"}`);
        console.log("");
      }
    }
  }
...
async function main() {
  try {
    const result = await addNewTools();
