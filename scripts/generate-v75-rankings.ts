...
/**
 * Generate November 2025 Rankings with Algorithm v7.5 (Balanced Technical + Capability Focus)
 *
 * This version combines:
 * 1. npm data quality fix (removed 15 incorrect mappings, 22.9M bogus downloads)
 * 2. Market-validated weights (40% adoption focus: Technical Performance 18%, Agentic Capability 12%)
 ...
 * 3. Calculate scores using v7.5 algorithm (market-validated weights + missing data penalty)
...
import { RankingEngineV75, ALGORITHM_V75_WEIGHTS } from "@/lib/ranking-algorithm-v75";
...
async function generateV75Rankings() {
  const db = getDb();
  console.log("\n🚀 Generating November 2025 Rankings with Algorithm v7.5 (Market-Validated + npm Fix)\n");
  console.log("=".repeat(80));
  console.log("\n📊 Algorithm v7.5 Weights (Market-Validated):");
  console.log(`   Agentic Capability:    ${ALGORITHM_V75_WEIGHTS.agenticCapability.toFixed(3)} (↓ from 0.100)`);
  console.log(`   Innovation:            ${ALGORITHM_V75_WEIGHTS.innovation.toFixed(3)} (↓ from 0.100)`);
  console.log(`   Technical Performance: ${ALGORITHM_V75_WEIGHTS.technicalPerformance.toFixed(3)} (unchanged)`);
  console.log(`   Developer Adoption:    ${ALGORITHM_V75_WEIGHTS.developerAdoption.toFixed(3)} (↑ from 0.150) 🔥`);
  console.log(`   Market Traction:       ${ALGORITHM_V75_WEIGHTS.marketTraction.toFixed(3)} (↑ from 0.120) 🔥`);
  console.log(`   Business Sentiment:    ${ALGORITHM_V75_WEIGHTS.businessSentiment.toFixed(3)} (↓ from 0.130)`);
  console.log(`   Development Velocity:  ${ALGORITHM_V75_WEIGHTS.developmentVelocity.toFixed(3)} (↓ from 0.140)`);
  console.log(`   Platform Resilience:   ${ALGORITHM_V75_WEIGHTS.platformResilience.toFixed(3)} (↓ from 0.140)`);
  ...
  // Step 3: Calculate scores with v7.5
  console.log("🧮 Calculating scores with Algorithm v7.5 (Market-Validated + Missing Data Penalty)...\n");
  const engine = new RankingEngineV75(ALGORITHM_V75_WEIGHTS);
  ...
  try {
    ...
    console.log("\n" + "=".repeat(80));
    console.log("✅ November 2025 Rankings Generated Successfully (v7.5)!");
    console.log("=".repeat(80));
...
generateV75Rankings()
  .then(() => {
    console.log("\n✨ Done! Rankings are now live with Algorithm v7.5 (market-validated + npm fix)\n");
    process.exit(0);
  })
  .catch((error) => {
    console.error("\n💥 Fatal Error:", error);
