...
async function validateProductivityUpdate(): Promise<void> {
  ...
  try {
    ...
    const productivityArticles = newsData.filter((article: any) => 
      article.tags && (
    ...
    const articlesWithImpact = productivityArticles.filter((article: any) => 
      article.impact_assessment && article.metadata?.productivity_paradox_research
    );
  ...
  try {
    ...
    const productivityEntries = metricsData.entries.filter((entry: any) => 
  ...
  try {
    const rankingsPath = path.join(dataDir, 'json', 'rankings', 'periods', '2025-07-14-productivity-adjusted.json');
  ...
  try {
    const newsBackup = path.join(dataDir, 'json', 'news', 'articles', '2025-07.json.backup-pre-productivity-update');
    const metricsBackup = path.join(dataDir, 'metrics-by-date', 'metrics-latest.json.backup-pre-productivity-update');
