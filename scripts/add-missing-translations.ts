...
function addMissingKeys(langCode: string, langDict: any, missingKeys: string[]) {
  const updates: any = {};
  let updatedCount = 0;
  ...
  if (missingKeys.some(key => key.startsWith('rankings.tierLegend'))) {
    ...
    if (!langDict.rankings.tierLegend && tierLegendTranslations[langCode]) {
      langDict.rankings.tierLegend = tierLegendTranslations[langCode];
      updatedCount++;
...
for (const lang of languages) {
  ...
  if (langReport && langReport.missingKeys && langReport.missingKeys.length > 0) {
    const { dict: updatedDict, updatedCount } = addMissingKeys(lang, langDict, langReport.missingKeys);
