...
async function testProductionDatabase() {
  ...
  const config = {
    ...
    DATABASE_URL: process.env["DATABASE_URL"] ? maskPassword(process.env["DATABASE_URL"]) : undefined,
    DATABASE_URL_UNPOOLED: process.env["DATABASE_URL_UNPOOLED"] ? maskPassword(process.env["DATABASE_URL_UNPOOLED"]) : undefined,
  ...
  if (errors.length > 0) {
    ...
    if (!process.env["DATABASE_URL"]) {
      log('\n💡 Tip: Make sure .env.production.local exists with your database credentials', colors.yellow);
      process.exit(1);
    }
  } else {
  ...
  try {
    const unpooledSql = neon(process.env["DATABASE_URL_UNPOOLED"]!);
