...
import { migrations, news, rankings, tools } from "../src/lib/db/schema";
...
async function migrateTools(db: any, stats: MigrationStats): Promise<void> {
  ...
  for (const tool of toolsData.tools) {
    try {
      ...
      const existing = await db.select().from(tools).where(eq(tools.slug, tool.slug)).limit(1);
      ...
      if (MODE === "dry-run") {
        ...
        if (existing.length > 0) {
          // Update existing
          await db.update(tools).set(toolRecord).where(eq(tools.slug, tool.slug));
...
async function migrateRankings(db: any, stats: MigrationStats): Promise<void> {
  ...
  try {
    ...
    const existing = await db.select().from(rankings).where(eq(rankings.period, period)).limit(1);
    ...
    if (MODE === "dry-run") {
      console.log(
        `  🔍 Would migrate rankings for period: ${period} (${rankingsData.rankings.length} tools)`
      );
    } else if (MODE === "migrate" || MODE === "sync") {
      if (existing.length > 0) {
        // Update existing
        await db.update(rankings).set(rankingRecord).where(eq(rankings.period, period));
  ...
  for (let i = 0; i < newsData.articles.length; i += BATCH_SIZE) {
    ...
    for (const article of batch) {
      try {
        ...
        if (MODE === "dry-run") {
          if (i < 5) {
            // Only show first 5 in dry-run
            console.log(`  🔍 Would migrate: ${article.title.substring(0, 60)}...`);
          }
        } else if (MODE === "migrate" || MODE === "sync") {
          if (existing.length > 0) {
            // Update existing
            await db.update(news).set(newsRecord).where(eq(news.slug, article.slug));
          } else {
            // Insert new
...
async function recordMigration(
  ...
  try {
    const existing = await db.select().from(migrations).where(eq(migrations.name, name)).limit(1);
    ...
    if (existing.length > 0) {
      await db.update(migrations).set(migrationRecord).where(eq(migrations.name, name));
    } else {
