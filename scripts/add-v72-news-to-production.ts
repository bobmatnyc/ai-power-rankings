...
/**
 * Add Algorithm v7.2 News Article to PRODUCTION
 *
 * This script adds ONLY the news article announcing Algorithm v7.2,
 * without modifying rankings (which already exist in production).
 */
...
console.log("📰 Adding Algorithm v7.2 News Article to PRODUCTION");
console.log("=".repeat(80));

const pool = new Pool({ connectionString: DATABASE_URL });
const db = drizzle(pool);

async function addV72News() {
  try {
    // Check if article already exists
    console.log("\n🔍 Checking if article already exists...");
    const existing = await db.select().from(news).where(eq(news.slug, "algorithm-v72-october-2025-rankings"));

    if (existing.length > 0) {
      console.log("⚠️  Article already exists in production!");
      console.log(`   Slug: ${existing[0].slug}`);
      console.log(`   Title: ${existing[0].title}`);
      console.log(`   Published: ${existing[0].publishedAt}`);
      console.log("\n✅ No action needed - article is already live");
      return;
    }

    console.log("✓ Article does not exist, proceeding with insert...\n");
...
`,
    ...
    console.log("✅ News article successfully added to PRODUCTION!");
    console.log("\n📊 Article Details:");
    ...
    console.log("\n🎉 Deployment Complete!");
    console.log("=".repeat(80));
    console.log("✅ Algorithm v7.2 announcement article is now live");
    console.log("🔗 View at: https://aipowerranking.com/en/news");
    console.log("📋 Direct link: https://aipowerranking.com/en/news/algorithm-v72-october-2025-rankings");

  } catch (error) {
    console.error("\n❌ Failed to add news article:", error);
    throw error;
  } finally {
    await pool.end();
  }
