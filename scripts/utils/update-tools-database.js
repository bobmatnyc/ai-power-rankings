...
const newCompanies = [
  {
    ...
    name: "Alibaba Group",
    description:
      "Chinese multinational technology company specializing in e-commerce, retail, internet, and AI technology",
...
newCompanies.forEach((newCompany) => {
  const exists = companies.companies.some(
    (c) => c.id === newCompany.id || c.slug === newCompany.slug
  );
  if (!exists) {
...
console.log(
  `- Updated ${Object.keys(toolUpdates).length} tools with company associations or pricing`
);
