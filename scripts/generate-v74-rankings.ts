...
/**
 * Generate November 2025 Rankings with Algorithm v7.4 (Market-Validated + npm Data Quality Fix)
 *
 * This version combines:
 * 1. npm data quality fix (removed 15 incorrect mappings, 22.9M bogus downloads)
 * 2. Market-validated weights (40% adoption focus: Developer Adoption 22%, Market Traction 18%)
 ...
 * 3. Calculate scores using v7.4 algorithm (market-validated weights + missing data penalty)
...
import { RankingEngineV74, ALGORITHM_V74_WEIGHTS } from "@/lib/ranking-algorithm-v74";
...
async function generateV74Rankings() {
  const db = getDb();
  console.log("\n🚀 Generating November 2025 Rankings with Algorithm v7.4 (Market-Validated + npm Fix)\n");
  console.log("=".repeat(80));
  console.log("\n📊 Algorithm v7.4 Weights (Market-Validated):");
  console.log(`   Agentic Capability:    ${ALGORITHM_V74_WEIGHTS.agenticCapability.toFixed(3)} (↓ from 0.100)`);
  console.log(`   Innovation:            ${ALGORITHM_V74_WEIGHTS.innovation.toFixed(3)} (↓ from 0.100)`);
  console.log(`   Technical Performance: ${ALGORITHM_V74_WEIGHTS.technicalPerformance.toFixed(3)} (unchanged)`);
  console.log(`   Developer Adoption:    ${ALGORITHM_V74_WEIGHTS.developerAdoption.toFixed(3)} (↑ from 0.150) 🔥`);
  console.log(`   Market Traction:       ${ALGORITHM_V74_WEIGHTS.marketTraction.toFixed(3)} (↑ from 0.120) 🔥`);
  console.log(`   Business Sentiment:    ${ALGORITHM_V74_WEIGHTS.businessSentiment.toFixed(3)} (↓ from 0.130)`);
  console.log(`   Development Velocity:  ${ALGORITHM_V74_WEIGHTS.developmentVelocity.toFixed(3)} (↓ from 0.140)`);
  console.log(`   Platform Resilience:   ${ALGORITHM_V74_WEIGHTS.platformResilience.toFixed(3)} (↓ from 0.140)`);
  ...
  // Step 3: Calculate scores with v7.4
  console.log("🧮 Calculating scores with Algorithm v7.4 (Market-Validated + Missing Data Penalty)...\n");
  const engine = new RankingEngineV74(ALGORITHM_V74_WEIGHTS);
  ...
  try {
    ...
    console.log("\n" + "=".repeat(80));
    console.log("✅ November 2025 Rankings Generated Successfully (v7.4)!");
    console.log("=".repeat(80));
...
generateV74Rankings()
  .then(() => {
    console.log("\n✨ Done! Rankings are now live with Algorithm v7.4 (market-validated + npm fix)\n");
    process.exit(0);
  })
  .catch((error) => {
    console.error("\n💥 Fatal Error:", error);
