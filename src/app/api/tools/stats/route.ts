...
export async function GET(request: NextRequest) {
  try {
    ...
    tools.forEach(tool => {
      ...
      // Status distribution
      if (tool.status) {
        stats.statusDistribution[tool.status] = 
          (stats.statusDistribution[tool.status] || 0) + 1;
      }
      
      // Pricing model distribution
      if (tool.pricing_model) {
        stats.pricingModelDistribution[tool.pricing_model] = 
          (stats.pricingModelDistribution[tool.pricing_model] || 0) + 1;
      }
      
      // License type distribution
      if (tool.license_type) {
        stats.licenseTypeDistribution[tool.license_type] = 
          (stats.licenseTypeDistribution[tool.license_type] || 0) + 1;
      }
      
      // Count tools with various properties
      if (tool.company_id) stats.toolsWithCompanies++;
      if (tool.github_repo) stats.toolsWithGithub++;
      if (tool.documentation_url) stats.toolsWithDocumentation++;
      
      // Count features and platforms
      if (tool.key_features) totalFeatures += tool.key_features.length;
      if (tool.supported_platforms) totalPlatforms += tool.supported_platforms.length;
    });
    
    // Calculate averages
    stats.averageFeaturesPerTool = tools.length > 0 
      ? parseFloat((totalFeatures / tools.length).toFixed(2))
      : 0;
    stats.averagePlatformsPerTool = tools.length > 0 
      ? parseFloat((totalPlatforms / tools.length).toFixed(2))
      : 0;
