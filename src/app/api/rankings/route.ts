...
const INNOVATIONS_MAP: Record<string, Innovation[]> = {
  'cursor': [{ score: 8, date: new Date('2024-03-01'), description: '.cursorrules for team standards' }],
  ...
  'claude-artifacts': [{ score: 8.5, date: new Date('2024-07-01'), description: 'Interactive component generation' }],
  'windsurf': [{ score: 7.5, date: new Date('2024-10-01'), description: 'Advanced multi-agent flows' }],
  ...
  'bolt-new': [{ score: 6.5, date: new Date('2024-08-01'), description: 'Full-stack app generation' }],
  ...
  'github-copilot': [{ score: 4, date: new Date('2023-06-01'), description: 'Multi-model support' }],
  'chatgpt-canvas': [{ score: 4.5, date: new Date('2024-10-01'), description: 'Interactive code editing' }],
  'diffblue-cover': [{ score: 6, date: new Date('2023-01-01'), description: 'Automated Java testing' }]
}
...
async function getToolMetrics(): Promise<ToolMetricsV6[]> {
  ...
  for (const metric of metrics) {
    ...
    if (!existingMetric || metric.recorded_at > existingMetric.recorded_at) {
      current[metric.metric_key] = {
        value: metric.value_integer || metric.value_decimal || metric.value_boolean || metric.value_json,
  ...
  for (const tool of tools) {
    ...
    toolMetricsV6.push({
      ...
      multi_file_capability: enhancements.multi_file_capability || getValue('multi_file_capability', 5),
