...
export function TierLegend({ dict }: TierLegendProps) {
  ...
  const description = dict?.rankings?.tierLegend?.description || "Tools are assigned tiers based on their ranking position";
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
          {tiers.map((tier) => (
            <div key={tier.name} className="flex flex-col gap-2">
              <div className="flex items-center gap-2">
                <Badge className={`${tier.color} font-bold px-3 py-1`}>
                  {tier.name}
                </Badge>
