...
export const ALGORITHM_V6_WEIGHTS: RankingWeightsV6 = {
  agenticCapability: 0.30,        // Primary differentiator
...
export class RankingEngineV6 {
  ...
  calculateQualityAdjustedRevenue(revenue: number, businessModel?: string): number {
    const multiplier = businessModel && REVENUE_QUALITY_MULTIPLIERS[businessModel] 
  ...
  calculatePlatformResilience(metrics: ToolMetricsV6): number {
    const baseResilience = (metrics.status === 'acquired' ? 3 : 7) * 0.5 +
  ...
  calculateDataCompleteness(metrics: ToolMetricsV6): number {
    ...
    const populated = coreMetrics.filter(key => 
   ...
   */
  validateToolForRanking(metrics: ToolMetricsV6): { isValid: boolean; completeness: number; confidence: number } {
    const completeness = this.calculateDataCompleteness(metrics);
    const confidence = this.calculateSourceConfidence(metrics);
    
    return {
      isValid: completeness >= VALIDATION_RULES.min_metrics_required && 
  ...
  static getAlgorithmInfo(): {
    ...
    return {
      ...
      description: 'Enhanced algorithm with innovation decay, platform risk, and revenue quality modifiers',
