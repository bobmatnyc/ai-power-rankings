            ...
            return {
              ...
              execute: async () => {
                ...
                switch (collection) {
                  case "tools":
                    return {
                      data: await payloadAPI.getTools({ where: { [field]: { equals: value } } }),
                      error: null,
                    };
                  case "rankings":
                ...
                return {
                  execute: async () => {
                    switch (collection) {
                      case "tools":
                        return {
                          data: (await payloadAPI.getTools({ limit, sort })).docs,
                          error: null,
                        };
                      case "rankings":
                        return {
                          data: (await payloadAPI.getRankings({ limit, sort })).docs,
                          error: null,
                        };
                      case "news":
                        return {
                          data: (await payloadAPI.getNews({ limit, sort })).docs,
                          error: null,
                        };
                      default:
                        return { data: null, error: new Error("Collection not implemented") };
