...
import { Subscriber, SubscribersData } from './schemas';
...
export class SubscribersRepository extends BaseRepository<SubscribersData> {
   ...
   */
  async create(subscriberData: Omit<Subscriber, 'id' | 'created_at' | 'updated_at'>): Promise<Subscriber> {
  ...
  async getStatistics(): Promise<{
    ...
    const last30Days = data.subscribers.filter(s => 
      new Date(s.created_at) >= thirtyDaysAgo && s.status === 'verified'
    ).length;
    
    const last7Days = data.subscribers.filter(s => 
      new Date(s.created_at) >= sevenDaysAgo && s.status === 'verified'
    ).length;
