...
export function ErrorFallback({ error, resetError }: { error: Error; resetError: () => void }) {
  return (
    <div className="bg-yellow-50 border border-yellow-200 rounded-lg shadow-md p-6">
      <h3 className="text-lg font-bold text-yellow-800 mb-2">Component Error</h3>
      <p className="text-sm text-yellow-700 mb-3">This component failed to load properly.</p>
      <div className="text-xs text-gray-600 bg-white p-2 rounded mb-3">{error.message}</div>
      <button
        type="button"
        onClick={resetError}
