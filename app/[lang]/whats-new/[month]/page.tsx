...
async function fetchSummaryByMonth(month: string): Promise<{
  summary: MonthlySummary;
  navigation: NavigationInfo;
}> {
  try {
    const baseUrl = getUrl();
    const response = await fetch(`${baseUrl}/api/whats-new/public?month=${month}`, {
    ...
    console.error("Error fetching summary for month:", month, error);
    notFound();
  }
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  try {
    const { summary } = await fetchSummaryByMonth(month);

    // Build hreflang alternates for all supported languages
    const languages: Record<string, string> = {};
    locales.forEach((locale) => {
      languages[locale] = `${baseUrl}/${locale}/whats-new/${month}`;
    });
    ...
    return {
      ...
      openGraph: {
        ...
        description,
        type: "article",
        url: `${baseUrl}/${lang}/whats-new/${month}`,
      ...
      alternates: {
        canonical: `${baseUrl}/en/whats-new/${month}`,
        languages,
      },
    };
  } catch {
...
export default async function WhatsNewMonthPage({ params }: PageProps): Promise<React.JSX.Element> {
  const { lang, month } = await params;
  ...
  // Fetch the specific month's summary
  const { summary, navigation } = await fetchSummaryByMonth(month);
