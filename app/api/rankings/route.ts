...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    const formattedRankings = await Promise.all(
      rankings.map(async (ranking: any) => {
        ...
        return {
          ...
          changeReason:
            ranking["change_analysis"]?.["primary_reason"] || ranking["change_reason"] || "",
          tool: {
            id: tool.id,
            slug: tool.slug,
            name: tool.name,
          ...
          scores: {
            ...
            news_impact:
              ranking["factor_scores"]?.["innovation"] || ranking["scores"]?.["news_impact"] || 0,
            agentic_capability:
    ...
    const toolsWithNews = validRankings.filter(
      (r) => r?.metrics?.news_articles_count && r.metrics.news_articles_count > 0
    ).length;
    ...
    const avgNewsBoost =
      newsImpacts.length > 0 ? newsImpacts.reduce((a, b) => a + b, 0) / newsImpacts.length : 0;

    const maxNewsImpact = newsImpacts.length > 0 ? Math.max(...newsImpacts) : 0;
